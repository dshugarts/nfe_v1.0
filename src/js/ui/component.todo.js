...
!function($) {
    ...
    TodoApp.prototype.generate = function() {
        ...
        for(var count=0; count<this.$todoData.length;count++) {
            ...
            var todoItem = this.$todoData[count];
            if(todoItem.done == true)
                this.$todoList.prepend('<li class="list-group-item border-0 pl-0"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input todo-done" id="' + todoItem.id + '" checked><label class="custom-control-label" for="' + todoItem.id + '"><s>' + todoItem.text + '</s></label></div></li>');
            else {
